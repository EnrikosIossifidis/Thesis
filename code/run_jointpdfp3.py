...
def run_jpdf(d,all_initials=False,no_test=False,multi=False):
    args = vars(d['args'])
    prev = None
    if d['args'].prev:
    ...
    for i in range(args['systems']):
        ...
        before = time.time()
        syn,p_XYS,data = synergistic_information(p_XY,variables_Y,variables_X,num_srvs=args['num_srvs'],tol_nonsyn_mi_frac=args['tol']
                                ,minimize_method=args['mm'],num_repeats_per_srv_append=args['n_repeats'],
                                summed_modulo=args['summed_modulo'],multi=multi,all_initials=initial_guess)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_test', default=False,type=lambda x: bool(strtobool(x)), help='Skip asserts and testing')
    ...
    # python3 only parameters
    parser.add_argument('--multi', default=False,type=lambda x: bool(strtobool(x)), help='Start with parity SRV or not')
    ...
    d['args'] = args
    d = run_jpdf(d,all_initials=args.all_initials,no_test=args.no_test,multi=args.multi)
