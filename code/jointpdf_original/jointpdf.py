...
class JointProbabilityMatrix(object):
    ...
    def synergistic_information(self, variables_Y, variables_X, tol_nonsyn_mi_frac=0.05, verbose=True,
        ...
        syn_entropy = self.synergistic_entropy_upper_bound(variables=variables_X)
        ...
        total_syn_mi = 0
        tot_vars = len(variables_Y)+len(variables_X)
        for i in xrange(max_num_srv_add_trials):
            ...
            # syn_vars = list(range(tot_vars,len(pdf_with_srvs)))
        ...
        syn_info = sum([pdf_with_srvs.mutual_information([srv_id],variables_Y)
                        for srv_id in xrange(len(self), len(pdf_with_srvs))])

        return syn_info, pdf_with_srvs
    ...
    def synergistic_entropy_upper_bound(self, variables=None):
        ...
        return float(self.entropy(variables) - max(indiv_entropies))
